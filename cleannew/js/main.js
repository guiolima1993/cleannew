...
if ($('.cookies-background').length) {
  $(document).on('click', '.cookies .dismiss-cookies', function(event) {
    ...
    $('.cookies-background').fadeOut('500', function() {
      ...
      Cookies.set('initial', true, {expires: 1});
    });
  });
}
...
$(function() {
    ...
    $(window).scroll(function() {
        ...
        var score_section = $('.score').closest('section').position().top;
        var wh = $(window).height();
        var scrollTop = $(window).scrollTop() + wh;

        if (score_done == false) {

            if (scrollTop >= score_section) {
                score_done = true;
                score();
            }
        }
    ...
    function score() {
        $('.score').each(function(index, elem) {
            var till = parseInt($(elem).data('size'));
            var start = parseInt($(elem).text());
            var n = [];
            n[index] = setInterval(function() {
                $(elem).text(start);
                start ++;
                if (start > till) {
                    clearInterval(n[index]);
                }
            }, 1);
        });
    }
